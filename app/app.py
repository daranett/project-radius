...
@app.route('/network-maps')
@login_required
def network_maps():
    return render_template('network_maps.html')
...
# === NETWORK MAPS & ODP/ODC API ===

@app.route('/api/network/odp')
@login_required
def api_network_odp():
    """Get all ODP devices"""
    try:
        # Cek dulu apakah tabel exists
        table_exists = execute_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
                AND table_name = 'network_odp'
            )
        """)
        
        if not table_exists[0]['exists']:
            return jsonify([])  # Return empty jika tabel tidak ada
            
        odp_list = execute_query("""
            SELECT * FROM network_odp 
            WHERE status = 'active'
            ORDER BY name
        """)
        return jsonify(odp_list)
    except Exception as e:
        print(f"DEBUG: ODP query error, returning empty: {str(e)}")
        return jsonify([])  # Return empty array instead of error

@app.route('/api/network/odc')
@login_required
def api_network_odc():
    """Get all ODC devices"""
    try:
        ...
        odc_list = execute_query("""
            SELECT * FROM network_odc 
            WHERE status = 'active'
            ORDER BY name
        """)
        return jsonify(odc_list)
    except Exception as e:
        print(f"DEBUG: ODC query error, returning empty: {str(e)}")
        return jsonify([])  # Return empty array instead of error

@app.route('/api/network/odp/<int:odp_id>/customers')
@login_required
def api_odp_customers(odp_id):
    """Get customers connected to specific ODP"""
    try:
        # Cek dulu apakah kolom odp_id exists di billing_customers
        column_exists = execute_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_schema = 'public' 
                AND table_name = 'billing_customers' 
                AND column_name = 'odp_id'
            )
        """)
        
        if not column_exists[0]['exists']:
            return jsonify([])  # Return empty jika kolom tidak ada
            
        customers = execute_query("""
            SELECT c.*, p.name as package_name
            FROM billing_customers c
            LEFT JOIN billing_packages p ON c.package_id = p.id
            WHERE c.odp_id = %s AND c.deleted_at IS NULL
            ORDER BY c.full_name
        """, (odp_id,))
        return jsonify(customers)
    except Exception as e:
        print(f"DEBUG: ODP customers error, returning empty: {str(e)}")
        return jsonify([])

@app.route('/api/customers/<int:customer_id>/ping', methods=['POST'])
@login_required
def api_ping_customer(customer_id):
    """Ping specific customer and return status"""
    try:
        # Cek dulu apakah kolom ip_address exists
        column_exists = execute_query("""
            SELECT EXISTS (
                SELECT FROM information_schema.columns 
                WHERE table_schema = 'public' 
        ...
        if not column_exists[0]['exists']:
            return jsonify({'success': False, 'error': 'Customer IP address not configured'})
        
        # Get customer IP address
        customer = execute_query("""
            SELECT ip_address FROM billing_customers 
            WHERE id = %s AND deleted_at IS NULL
        """, (customer_id,))
        
        if not customer or not customer[0]['ip_address']:
            return jsonify({'success': False, 'error': 'Customer IP address not configured'})
        
        ip_address = customer[0]['ip_address']
        
        # Simple ping implementation
        import subprocess
        try:
            result = subprocess.run(
                ['ping', '-c', '3', '-W', '2', str(ip_address)],
                capture_output=True, text=True, timeout=10
            )
            is_online = result.returncode == 0
            
            # Update customer status jika kolom is_online exists
            is_online_exists = execute_query("""
                SELECT EXISTS (
                    SELECT FROM information_schema.columns 
                    WHERE table_schema = 'public' 
                    AND table_name = 'billing_customers' 
            ...
            if is_online_exists[0]['exists']:
                execute_query("""
                    UPDATE billing_customers 
                    SET is_online = %s, last_ping = CURRENT_TIMESTAMP
                    WHERE id = %s AND deleted_at IS NULL
                """, (is_online, customer_id), fetch=False)
            ...
            if is_online_exists[0]['exists']:
                execute_query("""
                    UPDATE billing_customers 
                    SET is_online = false, last_ping = CURRENT_TIMESTAMP
                    WHERE id = %s AND deleted_at IS NULL
                """, (customer_id,), fetch=False)
        ...
        print(f"DEBUG: Ping error: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_network_status():
    ...
    try:
        # Query yang aman - tidak menggunakan kolom is_online
        stats = execute_query("""
            SELECT 
                (SELECT COUNT(*) FROM billing_customers WHERE deleted_at IS NULL) as total_customers,
                0 as online_customers,  -- Default value untuk sementara
                COALESCE((SELECT COUNT(*) FROM network_odp WHERE status = 'active'), 0) as total_odp,
                COALESCE((SELECT COUNT(*) FROM network_odc WHERE status = 'active'), 0) as total_odc
        """)
        ...
        print(f"DEBUG: Network status error: {str(e)}")
        # Return basic stats tanpa error
        return jsonify({
            'total_customers': 0,
            'online_customers': 0, 
            'total_odp': 0,
...
# === ODP/ODC MANAGEMENT API ===

@app.route('/api/network/odp/add', methods=['POST'])
@login_required
def api_add_odp():
    """Add new ODP"""
    try:
        ...
        execute_query("""
            INSERT INTO network_odp (name, latitude, longitude, capacity, address, notes)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (
            data['name'],
            data.get('lat'),
            data.get('lng'),
            data.get('capacity', 16),
            data.get('address', ''),
            data.get('notes', '')
        ), fetch=False)
        
        return jsonify({'success': True, 'message': 'ODP added successfully'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/network/odc/add', methods=['POST'])
@login_required
def api_add_odc():
    """Add new ODC"""
    try:
        ...
        execute_query("""
            INSERT INTO network_odc (name, latitude, longitude, capacity, address, notes)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (
            data['name'],
            data.get('lat'),
            data.get('lng'),
            data.get('capacity', 48),
            data.get('address', ''),
            data.get('notes', '')
        ), fetch=False)
        
        return jsonify({'success': True, 'message': 'ODC added successfully'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
